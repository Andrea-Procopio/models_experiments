...
- Uses general model interface (SegFormer, DETR, MaskRCNN) instead of DETR only
- Computes area change ratios between before/after masks
- Generates threshold-based detection analysis (0.1% to 2.0% thresholds)
...
SUPPORTED MODELS:
- SegFormer: nvidia/segformer-b0-finetuned-ade-512-512, nvidia/segformer-b1-finetuned-ade-512-512, etc.
- DETR: facebook/detr-resnet-50-panoptic, facebook/detr-resnet-101-panoptic, etc.
- MaskRCNN: maskrcnn_resnet50_fpn, maskrcnn_resnet101_fpn, etc.

Usage:
    python exp3Change.py --model_interface segformer --images_dir /path/to/raw_images --output_dir /path/to/output [--resume]
    python exp3Change.py --model_interface detr --model_name facebook/detr-resnet-50-panoptic --images_dir /path/to/raw_images
    python exp3Change.py --model_interface maskrcnn --model_name maskrcnn_resnet50_fpn --images_dir /path/to/raw_images
"""
...
class ChangeDetectionExperiment:
    ...
    6. Analyzes detection rates across multiple thresholds (0.1% to 2.0%)
    ...
    - SegFormerInterface: For SegFormer models (nvidia/segformer-*)
    - DetrInterface: For DETR models (facebook/detr-*)
    - MaskRCNNInterface: For Mask R-CNN models (maskrcnn_*)
    - Note: SAM support requires interface wrapper (not currently integrated)
    """
    
    def __init__(self, model_interface: Any, output_dir: str,
                iou_threshold: float = 0.1,
                max_size_ratio: float = 7.0,
                min_size_ratio: float = 0.1,
                logger: logging.Logger = None):
        self.iou_threshold = iou_threshold
        
        # Adjust size filtering based on model type
        if hasattr(model_interface, 'model_name') and 'detr' in model_interface.model_name.lower():
            # DETR models generate smaller, more precise masks
            self.max_size_ratio = 10.0  # Allow larger masks
            self.min_size_ratio = 0.05  # Allow smaller masks
        else:
    ...
    def _setup_logger(self) -> logging.Logger:
        ...
        fh.setLevel(logging.INFO)  # Changed from DEBUG to INFO to reduce verbosity
    ...
    def _get_model_prefix(self) -> str:
        """Get the model prefix based on the model interface type."""
        model_type = type(self.model_interface).__name__.replace('Interface', '').lower()
    ...
    def _find_image_pairs(self, images_dir: str) -> List[Tuple[str, str, str]]:
        ...
        For no_change trials (no _out file), use the _init image for both before and after.
        (or if uncommented below Excludes no_change trials (no _out file) as per original logic.)
        """
        pairs = []
        ...
        for init_file in init_files:
            ...
            if out_file:
                # Regular change trial: init -> out
                out_path = os.path.join(images_dir, out_file)
                pairs.append((base_name, init_path, out_path))
                self.logger.debug(f"Found change pair: {base_name} -> {init_file} & {out_file}")
            else:
                # No_change trial: use the same image for both before and after
                pairs.append((base_name, init_path, init_path))
                self.logger.info(f"Found no_change pair: {base_name} -> {init_file} (using same image for both)")
                
                ## No_change trial: exclude from analysis (following original logic)
                #self.logger.debug(f"Excluding no_change trial: {base_name} -> {init_file} (no _out file found)")
    ...
    def _save_original_segmentation_overlay(self, frame: np.ndarray, output_dir: str, image_name: str) -> None:
        ...
        try:
            ...
            # Use the model interface's visualization method if available
            if hasattr(self.model_interface, 'visualize_predictions'):
                # Get raw predictions
                predictions = self.model_interface.infer_image(pil_image)
                
                # Use the model's own visualization method
                vis_image = self.model_interface.visualize_predictions(pil_image, predictions)
                
                # Save the visualization
                overlay_file = os.path.join(output_dir, f"{image_name}_original_segmentation.png")
                vis_image.save(overlay_file)
                
                self.logger.info(f"    Saved original segmentation overlay -> {overlay_file}")
                
            else:
                # Fallback: create a simple overlay using the raw predictions
                predictions = self.model_interface.infer_image(pil_image)
                pred_masks = predictions['pred_masks']  # Shape: (1, num_queries, H, W)
                
                # Create a simple overlay by averaging all masks
                if pred_masks.shape[1] > 0:
                    # Take the first few masks and create a simple overlay
                    num_masks_to_show = min(5, pred_masks.shape[1])
                    overlay = np.asarray(pil_image).copy()
                    
                    for i in range(num_masks_to_show):
                        mask = pred_masks[0, i].cpu().numpy()  # (H, W)
                        binary_mask = (mask > 0.5).astype(bool)
                        
                        if binary_mask.sum() > 50:  # Only show masks with some content
                            # Apply color based on mask index
                            color = [(255, 0, 0), (0, 255, 0), (0, 0, 255), (255, 255, 0), (255, 0, 255)][i % 5]
                            overlay[binary_mask] = np.array(color)
                    
                    # Blend with original image
                    blend = (0.6 * np.asarray(pil_image) + 0.4 * overlay).astype(np.uint8)
                    ...
                    self.logger.warning(f"No masks found for {image_name}")
            
        except Exception as e:
            self.logger.warning(f"Failed to save original segmentation overlay for {image_name}: {e}")

    def _detect_blob(self, frame: np.ndarray, thresholds: Tuple[int, ...] = (15, 10, 5)) -> Optional[np.ndarray]:
        """
        Detect blob using intensity thresholding (optimized for 1000x400 images with black background).
        Lower thresholds are used since we have black background and want to catch subtle blob boundaries.
        """
        #use proper grayscale conversion instead of naive sum
        #option 1: use OpenCV if available (most accurate)
        try:
            import cv2
            gray = cv2.cvtColor(frame, cv2.COLOR_RGB2GRAY)
        except ImportError:
            # Option 2: Use proper luminance conversion (standard formula)
        ...
        for thr in thresholds:
            ...
            if regs:
                # Additional filtering for 1000x400 images: ensure blob is not too small
                image_area = frame.shape[0] * frame.shape[1]
                min_blob_area = max(1000, image_area * 0.002)  # At least 1000 pixels or 0.2% of image
                
                if regs[0].area > min_blob_area:
                    self.logger.debug(f"    Blob detected with threshold {thr} (area={regs[0].area})")
                    blob_mask = (labeled == regs[0].label).astype(bool)
                    return blob_mask
                else:
                    self.logger.debug(f"    Blob too small at threshold {thr} (area={regs[0].area}, min={min_blob_area})")
            else:
        ...
        for i in range(pred_masks.shape[1]):
            ...
            for lbl in range(1, labeled.max() + 1):
                ...
                if component_mask.sum() > 0:
                    if component_mask.sum() > 50:  # Minimum area
    ...
    def _choose_best_mask(self, blob: np.ndarray, candidates: List[np.ndarray]) -> Tuple[Optional[np.ndarray], Optional[float]]:
        ...
        ##### BEFORE #####
        ##### ----------------------------------------------------------
        #before the model would always pick the best candidate even if IoU is very low
        #no minimum IoU threshold to reject poor matches
        #no size filtering --> can select massive masks that barely overlap with tiny blobs (b3 problem)
        ##### ----------------------------------------------------------
        ...
        for mask in candidates:
            mask_area = mask.sum()
            size_ratio = mask_area/blob_area if blob_area>0 else float('inf')
            if self.min_size_ratio <= size_ratio <= self.max_size_ratio:
                filtered_candidates.append(mask)
            else:
                rejected_count += 1
        
        # Only log summary of rejections to avoid spam
        if rejected_count > 0:
            self.logger.debug(f'Rejected {rejected_count}/{len(candidates)} masks due to size filtering')
        
        if not filtered_candidates:
            self.logger.warning('All candidates rejected due to filtering')
            return None, None
        
        #compute IoU for filtered candidates
        #cost == -IoU --> changed to maximize IoU instead of minimize cost for simplicity
        ious = np.array([self._compute_iou(blob, mask) for mask in filtered_candidates], dtype=np.float32)
        best_idx = int(ious.argmax())
        best_iou = float(ious[best_idx])

        # Log only the best IoU, not all values
        self.logger.info(f"    Best IoU: {best_iou:.3f} (candidate {best_idx})")

        #reject if IoU is too low
        if best_iou < self.iou_threshold:
            self.logger.warning(f"Best IoU {best_iou:.3f} below threshold {self.iou_threshold}, rejecting all candidates")
            return None, None
        
        return filtered_candidates[best_idx].astype(bool), -best_iou
    ...
    def _is_image_already_processed(self, image_name: str) -> bool:
        ...
        model_prefix = self._get_model_prefix()
        processed_dir = Path(self.processed_images_dir) / f"{model_prefix}_{image_name}"
    ...
    def _perform_threshold_analysis(self, pair_details: List[Dict[str, Any]]) -> None:
        ...
        # Custom threshold strategy: 0.1% to 5% in 0.1% steps, then 6% to 20% in 1% steps
        thresholds = np.concatenate([
            np.arange(0.001, 0.051, 0.001),  # 0.1%..5.0%
            np.arange(0.06, 0.201, 0.01)     # 6%..20%
        ])
        ...
        for thr in thresholds:
            # Use one-decimal precision labels (e.g., 0.1, 0.2, ..., 5)
            pct_label = f"{thr * 100:.1f}".rstrip('0').rstrip('.')
            self.logger.info(f"Analyzing threshold: {pct_label}%")
            
            # Create output directory for this threshold
            dir_out = os.path.join(self.threshold_results_dir, f"{pct_label}_comparison")
            ...
            self._generate_threshold_plots(detections, types, pct_label, dir_out)
        
        self.logger.info(
            f"Completed thresholds: {[f'{t*100:.1f}'.rstrip('0').rstrip('.') for t in thresholds]}%"
        )
    ...
    def _compute_sem_binary(self, detections: np.ndarray) -> float:
        ...
        NOTE: Temporarily removed downscaling factor (* 0.5) to debug SEM values.
        If SEMs are now different, the issue was that downscaling made them too small.
        """
        n = detections.size
        if n == 0:
            return 0.0
        ...
        # Apply requested downscaling factor
        result = sem * 0.5
        
        # DEBUG: Print intermediate values
        self.logger.debug(f"SEM computation: n={n}, p={p:.3f}, sem={sem:.3f}, final={result:.3f}")
        
        return result

    def _generate_threshold_plots(self, detections: Dict[str, List[int]], types: List[str], 
                                pct: str, output_dir: str) -> None:
        """
        ...
        # DEBUG: Print SEM values for each category
        self.logger.info(f"DEBUG - Threshold {pct}% SEM values:")
        for i, t in enumerate(types):
            n_samples = len(detections[t])
            rate = rates[i]
            sem = sems[i]
            self.logger.info(f"  {t}: n={n_samples}, rate={rate:.1f}%, sem={sem:.3f}")
...
def main():
    ...
    parser.add_argument("--images_dir", type=str, required=False,
                      default="/Users/andreaprocopio/Desktop/object_reps_neural/detr/EXP_3_CHANGE/Data_processed/Stimuli/Exp3b_Images",
                      help="Directory containing raw image files with _init and _out pairs")
    parser.add_argument("--output_dir", type=str, required=False,
                      default="/Users/andreaprocopio/Desktop/object_reps_neural/hugging_face/model_experiments/exp3b_results",
    ...
    if args.model_interface == "segformer":
        ...
        model_interface.set_detection_sensitivity("max")
    else:
